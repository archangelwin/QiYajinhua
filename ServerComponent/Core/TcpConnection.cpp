...
void SocketItem::SendData(char *pData)
{
	std::string str(pData);
	tcp_connection_->Sokcet().write_some(boost::asio::buffer(str,str.size()));
	//boost::asio::async_write(tcp_connection_->Sokcet(),boost::asio::buffer(str, str.size())  );
}

Tcp_Connection::pointer SocketItem::GetPoint()
{
